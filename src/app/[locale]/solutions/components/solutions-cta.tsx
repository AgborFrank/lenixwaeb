...
export default async function SolutionsCta() {
  const t = await getTranslations("Solutions.Cta");
  ...
  return (
    <section className={`${home.section} ${home.sectionMuted}`}>
      <div className={`${home.container} max-w-2xl text-center`}>
        ...
        <p className={`${home.lead} mb-8`}>{t("description")}</p>
        <div className="flex flex-wrap justify-center gap-3">
          <Link href="/contact" className={home.btnPrimary}>
            {t("btn_primary")}
          </Link>
          <Link href="/crypto-recovery#start-recovery" className={home.btnSecondary}>
            {t("btn_secondary")}
          </Link>
        </div>
      </div>
