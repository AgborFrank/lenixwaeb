...
export default async function BankingFinanceCta() {
  const t = await getTranslations("BankingFinance.Cta");

  return (
    <section className="py-24 bg-black relative">
      <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
        <h2 className="text-4xl md:text-6xl font-semibold text-white mb-6">{t("title")}</h2>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 mb-8">
          ...
          <Link href="/login" className={`${home.btnSecondary} h-12 px-10 text-base`}>
            {t("btn_secondary")}
          </Link>
        </div>
        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm">
          <Link href="/crypto-loan" className={home.textLink}>
            {t("links.loan")}
          </Link>
          <Link href="/wallet" className={home.textLink}>
            {t("links.wallet")}
          </Link>
