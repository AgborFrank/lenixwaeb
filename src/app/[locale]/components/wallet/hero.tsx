...
export default function WalletHero() {
  const t = useTranslations("WalletPage.Hero");
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            ...
            <p className="text-neutral-400 text-lg leading-relaxed max-w-lg">{t("subtitle")}</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href={DOWNLOAD_URL} target="_blank" className={home.btnPrimary}>
                {t("btn_download")}
              </Link>
              <a href="#features" className={home.btnSecondary}>
                {t("btn_learn")}
              </a>
            </div>
          </div>
              ...
              <div className="w-full h-full bg-gradient-to-b from-zinc-900 to-black text-white overflow-hidden flex flex-col pt-12">
                ...
                <div className="px-6 flex justify-between items-center mb-6">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-400 to-yellow-600 flex items-center justify-center font-bold text-black text-xs shadow-lg shadow-yellow-500/20">
                      L
                    </div>
                    <span className="font-semibold text-sm">{t("mock.account")}</span>
                ...
                <div className="px-6 text-center mb-8">
                  <p className="text-gray-400 text-xs mb-1">{t("mock.total_balance")}</p>
                  ...
                  <div className="inline-flex items-center gap-1 bg-green-500/10 px-2 py-1 rounded-full border border-green-500/20">
                    ...
                    <span className="text-xs font-medium text-green-400">{t("mock.change_24h")}</span>
                  </div>
                </div>

                <div className="px-6 grid grid-cols-4 gap-2 mb-8">
                  {[
                    { icon: <ArrowUpRight className="w-5 h-5" />, label: t("mock.send") },
                    { icon: <ArrowDownLeft className="w-5 h-5" />, label: t("mock.receive") },
                    { icon: <Repeat className="w-5 h-5" />, label: t("mock.swap") },
                    { icon: <Landmark className="w-5 h-5" />, label: t("mock.loan") },
                  ].map((action, i) => (
                ...
                <div className="px-6 flex gap-6 border-b border-white/5 mb-4">
                  <button className="pb-3 border-b-2 border-yellow-400 text-sm font-medium text-white">
                    {t("mock.tab_assets")}
                  </button>
                  <button className="pb-3 border-b-2 border-transparent text-sm font-medium text-gray-500">
                    {t("mock.tab_history")}
                  </button>
                </div>
                  ...
                  {MOCK_ASSETS.map((asset, i) => (
                    <div key={i} className="flex justify-between items-center group cursor-pointer">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full ${asset.icon} flex items-center justify-center text-white font-bold text-[10px] shadow-sm`}
                        >
                          {asset.ticker[0]}
                        </div>
                        <div>
