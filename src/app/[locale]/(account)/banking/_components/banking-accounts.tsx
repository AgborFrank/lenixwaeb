    ...
    <div className="grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2">
      <section className="rounded-lg border border-zinc-800 bg-zinc-950">
        <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3.5 sm:px-5 sm:py-4">
          <div>
            <h2 className="text-sm sm:text-base font-semibold text-white">{t("linked_title")}</h2>
            <p className="mt-1 text-xs sm:text-sm text-zinc-500">{t("linked_subtitle")}</p>
          </div>
          <button
            type="button"
            onClick={() => setAddOpen(true)}
            className="inline-flex h-10 items-center gap-1.5 rounded-md bg-zinc-800 px-3 text-xs font-semibold text-white transition-colors hover:bg-zinc-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FCD535]/50"
          >
        ...
        {accounts.length === 0 ? (
          <div className="px-4 py-8 sm:px-5 sm:py-10 text-center">
            <span className="mx-auto flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full bg-zinc-900 text-zinc-400">
              <Image src="/assets/vectors/trans.svg" width={200} height={200} alt="web" />
            </span>
          ...
          <div className="divide-y divide-zinc-800">
            {accounts.map((account) => (
              <div key={account.id} className="flex items-center gap-3 px-4 py-3.5 sm:px-5 sm:py-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-sm font-semibold text-white" title={account.bankName}>{account.bankName}</p>
                    {account.isDefault && (
                  ...
                  <p className="mt-1 truncate text-xs text-zinc-500" title={`${account.accountType} · ${account.maskedNumber} · ${account.currency}`}> 
                ...
                <span className="shrink-0"><StatusBadge status={account.status} /></span>
              </div>
            ))}
          </div>
        )}
      ...
      <section className="rounded-lg border border-zinc-800 bg-zinc-950">
        <div className="flex items-start justify-between border-b border-zinc-800 px-4 py-3.5 sm:px-5 sm:py-4">
          <div>
            <h2 className="text-sm sm:text-base font-semibold text-white">{t("limits_title")}</h2>
            <p className="mt-1 text-xs sm:text-sm text-zinc-500">{t("limits_subtitle")}</p>
          </div>
        ...
        <div className="space-y-4 sm:space-y-5 p-4 sm:p-5">
          {limits.map((limit) => {
            ...
            return (
              <div key={limit.label}>
                <div className="flex items-center justify-between gap-3 sm:gap-4 text-xs sm:text-sm">
                  <span className="text-zinc-300">{limit.label}</span>
                  <span className="font-medium tabular-nums text-white">
                    {new Intl.NumberFormat(locale, { style: "currency", currency: limit.currency, maximumFractionDigits: 0 }).format(limit.used)} /{" "}
                    {new Intl.NumberFormat(locale, { notation: "compact", style: "currency", currency: limit.currency }).format(limit.total)}
                  </span>
                </div>
          ...
          <div className="rounded-lg bg-zinc-900 px-3 py-3 text-xs leading-relaxed text-zinc-400 mt-1">
            {t("limits_note")}
          </div>
        </div>
      </section>
