    ...
    <div className="bg-zinc-900 border border-white/5 rounded-2xl p-6 h-full flex flex-col">
      ...
      <Tabs defaultValue="borrow" className="flex-1 flex flex-col">
         ...
         <TabsContent value="borrow" className="space-y-6 flex-1 flex flex-col">
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-zinc-500 px-1">
                     <span>Balance: 1.245 BTC</span>
                     <button className="text-yellow-400 hover:text-yellow-300">Max</button>
                  </div>
               </div>
            </div>
            ...
            <div className="space-y-4 py-2">
               <div className="flex justify-between items-center mb-2">
                  <label className="text-sm "><span className="text-zinc-400">Loan to Value (LTV):</span> <span className="text-white font-bold ml-1">{ltv[0]}%</span></label>
               ...
               <div className="flex justify-between text-[10px] text-zinc-600 font-mono">
                  <span>0%</span>
                  <span>50%</span>
                  <span className="text-red-400">85% Max</span>
               </div>
            </div>

            {/* Loan Summary */}
            <div className="bg-zinc-950/30 rounded-xl p-4 space-y-3 border border-white/5 mt-auto">
               <SummaryRow label="Annual Interest (APY)" value="5.5%" highlight />
               <SummaryRow label="Liquidation Price" value="$28,450.00" sublabel="(BTC)" />
               <SummaryRow label="Origination Fee" value="0.5%" />
               <div className="h-px bg-white/5 my-2" />
               <div className="flex justify-between items-center">
                  <span className="text-sm text-zinc-300">Total Repayment</span>
                  <span className="text-lg font-bold text-white">$12,512.25</span>
               </div>
            </div>

            <Button size="lg" className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-medium h-12 shadow-lg shadow-yellow-500/20">
               Confirm Borrow
            </Button>
         </TabsContent>
         ...
         <TabsContent value="repay" className="space-y-6 text-center py-10">
            ...
            <h3 className="text-lg font-medium text-white">No active loans to repay</h3>
            <p className="text-sm text-zinc-500 max-w-xs mx-auto">
               You don't have any outstanding loans. Borrow first to see repayment options here.
            </p>
            <Button variant="outline" onClick={() => document.querySelector('[value="borrow"]')?.dispatchEvent(new Event('click', { bubbles: true }))}>
               Start Borrowing
            </Button>
         </TabsContent>
      </Tabs>
